...
- **listFolders pagination** — folders with >1000 entries no longer truncate
  (v0.0.6).
- **Bucket-wide server-side search** — recursive search finds files in
  collapsed/unloaded folders (v0.0.16).
- **Recursive folder upload** — dragged folders recreate their structure
  (v0.0.17).
- **F2 to rename / double-click to open** (v0.0.18).
...
3. **Transfer queue with cancel + parallelism** — uploads/downloads run one at
...
4. **Overwrite / conflict prompt on upload** — the `exists()` API already
   exists but is unused; warn before clobbering.
5. **Inline text-file editing + "New file"** — create/edit small
   text/JSON/config files in place.
6. **Object versioning** — list and restore previous versions (when the bucket
